...
from wtforms import StringField, PasswordField, SelectField, TextAreaField, DateField, TimeField, IntegerField, DecimalField, BooleanField, EmailField
...
class ExperimentalClassForm(FlaskForm):
    name = StringField('Nome Completo', validators=[DataRequired(), Length(max=200)])
    email = EmailField('E-mail', validators=[DataRequired(), Email()])
    phone = StringField('Telefone', validators=[DataRequired(), Length(max=20)])
    instrument = StringField('Instrumento de Interesse', validators=[DataRequired(), Length(max=100)])
    ...
    preferred_time = StringField('Horário Preferencial', validators=[Optional(), Length(max=50)])
    notes = TextAreaField('Observações', validators=[Optional()])
